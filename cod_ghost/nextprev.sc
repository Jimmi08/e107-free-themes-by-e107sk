...
if($total_pages > 1)
{
    if(isset($pref['old_np']) && $pref['old_np'])
    {
        ...
        if(!defined("NEXTPREV_NOSTYLE") || NEXTPREV_NOSTYLE==FALSE){
            $NP_PRE_ACTIVE = " ";
            $NP_POST_ACTIVE = " ";
            $NP_STYLE = "style='text-decoration: none'";
        }
        ...
        $nppage .= "\n\n<!-- Start of Next/Prev -->\n\n";
        $np_parms['prev'] = "&laquo;";
        $np_parms['np_class'] = 'nextprev_link';
        ...
        if($current_page > 1)
        {
          ...
          $nppage .= "<a class='{$np_parms['np_class']}' style='text-decoration:none' href='{$link}'>{$np_parms['prev']}</a>";
        }
        if ($total_pages > 10)
        {
            $current_page = ($current_start/$perpage)+1;
        ...
        if($current_page < $total_pages)
        {
          ...
          $nppage .= "<a class='{$np_parms['np_class']}' style='text-decoration:none' href='{$link}'>{$np_parms['next']}</a>";
        }
        $nppage .= "\n\n<!-- End of Next/Prev -->\n\n";
        return "<span class='nextprev_total'>$caption</span>".$nppage;
    }

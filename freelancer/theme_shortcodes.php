...
class theme_shortcodes extends e_shortcode
{
	...
	function sc_bootstrap_usernav($parm='')
	{
		...
		if( e107::isInstalled('pm') )
		{
			$text .= '<li class="nav-item mx-0 mx-lg-1 dropdown">{THEME_PM_NAV}</li>';  
		}        
        
        $text .= '
			<li class="nav-item mx-0 mx-lg-1 dropdown">
			<a href="#" class="nav-link py-3 px-0 px-lg-3 dropdown-toggle" data-toggle="dropdown">{SETIMAGE: w=20} {USER_AVATAR: shape=circle} '. USERNAME.' <b class="caret"></b></a>
			<ul class="dropdown-menu nav-item dropdown">
				<li class="nav-item dropdown"><a class="dropdown-item" href="{LM_USERSETTINGS_HREF}"><i class="fas fa-cog"></i> '.LAN_SETTINGS.'</a></li>
				<li class="nav-item dropdown"><a class="dropdown-item no-block" role="button" href="{LM_PROFILE_HREF}"><i class="fas fa-user"></i> '.LAN_LOGINMENU_13.'</a></li>';
			
		if(ADMIN) 
		{
			$text .= '<li class="nav-item dropdown"><a class="dropdown-item" href="'.e_ADMIN_ABS.'"><i class="fas fa-cogs"></i> '.LAN_LOGINMENU_11.'</a></li>';	
		}
		
		$text .= '<li class="nav-item dropdown"><a class="dropdown-item" href="'.e_HTTP.'index.php?logout"><i class="fas fa-sign-out-alt"></i> '.LAN_LOGOUT.'</a></li>
			</ul>
			</li>
		';
	...
	function sc_theme_pm_nav($parm = NULL)
	{
		...
		return '<a class="nav-link py-3 px-0 px-lg-3 dropdown-toggle '.$class.'"  data-toggle="dropdown" href="#">'.$icon.$count.'</a>
			<ul class="dropdown-menu nav-item dropdown">
				<li class="nav-item dropdown"><a class="dropdown-item" href="'.$urlInbox.'">'.LAN_PLUGIN_PM_INBOX.'</a></li>
				<li class="nav-item dropdown"><a class="dropdown-item" href="'.$urlOutbox.'">'.LAN_PLUGIN_PM_OUTBOX.'</a></li>
				<li class="nav-item dropdown"><a class="dropdown-item" href="'.$urlCompose.'">'.LAN_PLUGIN_PM_NEW.'</a></li>
			</ul>';
	}	
}
